					...
					<div key={i}>
						{(i === 0 || message.user !== chat.chatHistory[i - 1].user) && (
							<div>
								<img
									className="userIcon"
									src={`${router.basePath}${USERS_ICON_URL[Object.keys(Users).includes(message.user.toUpperCase()) ? (message.user.toUpperCase() as keyof typeof Users) : Users.UNKNOWN]}`}
									alt="userIcon"
								/>
								<span className="userName" key={`user-${i}`}>
									{message.user[0] + message.user.slice(1).toLowerCase()}
								</span>
							</div>
						)}
				...
				<>
					...
					<div className="buttons">
						...
						{gameMode === GameMode.NEBULA && (
							<div className="nebulaWrapperButtons">
								<button
									className={
								...
								<button
									className="realisticButton download"
									onMouseDown={() => {
