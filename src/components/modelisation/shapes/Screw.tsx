...
function Screw({ pending, position, color }: shapeComponentProps) {
	...
	const screwGeometry = useMemo(() => {
		...
		const hemisphere = new THREE.Mesh(new THREE.SphereGeometry(0.5, 32, 16, 0, Math.PI * 2, 0, Math.PI / 2));
		...
		const resultCSG = hemisphereCSG.subtract(box1CSG).subtract(box2CSG);
