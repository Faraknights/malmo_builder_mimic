...
export class ShapeGroup{
    ...
    constructor(startingPoint: CartesianCoordinate, shapes: (shapeProps | ShapeGroup)[], pending: boolean, breakable?: boolean) {
        this.startingPoint = startingPoint;
        this.shapes = shapes.map(shape => ({...shape, breakable: !!breakable}))
        this.uuid = uuidv4()
    ...
    getPositions = (): CartesianCoordinate[] => {
        return this.shapes.flatMap(shape => {
            if(shape instanceof ShapeGroup){
                ...
                return coordinateAddition(shapeHitbox[shape.shape] as CartesianCoordinate[], shape.position) as CartesianCoordinate[]
            }
        })
    }
};
        ...
        return shapes.map((shape, index) => {
            if (shape instanceof ShapeGroup) {
                return (
                    <Group
                        key={`group-${index}`}
                        shapeGroup={{...shape, breakable: shapeGroup.breakable}}
                    />
                );
            } else {
                return <Shapes
