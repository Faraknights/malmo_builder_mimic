...
export function parseMinecraftInstruction(instruction: string, currentWorldState: WorldStateProps): WorldStateProps {
	...
	if (cleanedInstruction.startsWith('place')) {
		...
		currentWorldState.shapeInPlace.push({
			...
			breakable: false,
			uuid: uuidv4(),
			color: COLORS[placement[1].toUpperCase() as keyof typeof COLORS] || COLORS.WHITE,
			position: {
				x: parseInt(placement[2], 10),
				y: parseInt(placement[3], 10),
				z: parseInt(placement[4], 10),
			},
