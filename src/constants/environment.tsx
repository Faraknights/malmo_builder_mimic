...
const isWithinLimits = (coordinates: CartesianCoordinate[], gridSize: gridSizeProps): boolean => {
    return coordinates.every(coord => 
        coord.x >= gridSize.x.min && coord.x <= gridSize.x.max &&
        coord.y >= gridSize.y.min && coord.y <= gridSize.y.max &&
        coord.z >= gridSize.z.min && coord.z <= gridSize.z.max
    );
};

const hasCoordinateBelow = (coordinate: CartesianCoordinate, placedCoordinates: CartesianCoordinate[]): boolean => {
    return coordinate.y === 1 || placedCoordinates.some(placedCoord =>
...
export const GAME_RULE: { [key in EnvironmentMode]: (
    shape: shapeProps,
    newCoordinates: CartesianCoordinate[], 
    placedCoordinates: CartesianCoordinate[], 
    skipCheckBelow?: boolean
) =>  shapeProps | undefined} = {
    [EnvironmentMode.MINECRAFT]: (shape, newCoordinates, placedCoordinates, skipCheckBelow?) => {
...
export const EXPORT_GAME_LOG: { [key in EnvironmentMode]: ( gameLog: GameLog ) => Object } = {
    [EnvironmentMode.MINECRAFT]: (gameLog: GameLog) => ({
        "WorldStates": gameLog.gameLog.map(worldState => ({
            "Timestamp": worldState.timestamp,
            "ChatHistory" : worldState.chatHistory.map(message => (
